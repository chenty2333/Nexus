...
function parse_service_line(    event, cookie, effect, before, after) {
    ...
    if (event == "BEGIN") {
        ...
        if ($0 != "LINUX_FS_SERVICE PASS real_user_service_crash=true fsd_v1_page_fault=true fsd_v2_post_crash_construction=true fsd_v2_fresh_task=true fsd_v2_fresh_vm=true current_task_key_bound=true crash_cohort=filesystem_read_only delayed_old_prepare=true stale_old_binding_full_projection_unchanged=true old_sender_current_handle_rejected=true device_commit_gate_after_rebind=true device_committed_after_rebind=false guest_reply_after_rebind=true reply_wakeups=1 exactly_once=true registry_quiescent=true bounded=true single_cpu=true")
            fail("malformed precommit filesystem-service PASS receipt: " $0)
...
function parse_abort(    ordinal, kind, effect) {
    ...
    if (ordinal > 3)
        add_effect(effect, kind)
    if (ordinal == 5 && effect != service_effect)
        fail("filesystem-read Abort does not match fsd-v1 Register")
...
function validate_serial() {
    ...
    if (!service_begin_line || !guest_blocked_line || !service_register_line ||
        ...
        !adopt_line || !stale_replay_line || !capture_line || !commit_gate_line ||
        !close_line || !dispatch_outcome_line || !v2_exit_line ||
        !guest_publication_line || !pass_line || !slice_pass_line ||
        !service_pass_line || !terminal_line || !spike_line || !fixture_line ||
        !(service_begin_line < guest_blocked_line &&
          ...
          stale_replay_line < capture_line && capture_line < commit_gate_line &&
          commit_gate_line < close_line && close_line < dispatch_outcome_line &&
          ...
          service_pass_line < terminal_line &&
          terminal_line < spike_line && spike_line < fixture_line))
        fail("filesystem service/precommit/publication order is incomplete or invalid")
}
...
function parse_debug_line(    vdev, queue, vq, iova) {
    sub(/\r$/, "")
    if ($0 ~ /^(LINUX_FS_|FSD_V[12] |SPIKE_RESULT |RUNTIME_FS_|EFFECT_REGISTRY )/)
