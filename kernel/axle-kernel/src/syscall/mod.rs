...
use axle_types::syscall_numbers::{
    ...
    AXLE_SYS_AX_VMAR_CLONE_MAPPINGS, AXLE_SYS_AX_VMAR_GET_MAPPING_VMO,
    AXLE_SYS_AX_VMO_CREATE_PRIVATE_CLONE, AXLE_SYS_AX_VMO_GET_INFO, AXLE_SYS_AX_VMO_LOOKUP_PADDR,
    AXLE_SYS_AX_VMO_PIN, AXLE_SYS_AX_VMO_PROMOTE_SHARED, AXLE_SYS_CHANNEL_CREATE,
...
pub const BOOTSTRAP_SYSCALLS: [SyscallNumber; 88] = [
    ...
    AXLE_SYS_AX_VMO_CREATE_PRIVATE_CLONE,
    ...
    AXLE_SYS_AX_VMAR_GET_MAPPING_VMO,
    ...
    AXLE_SYS_AX_PCI_DEVICE_SET_COMMAND,
...
#[derive(Debug)]
struct ConsoleWriteRequest {
    bytes: Vec<u8>,
}

fn decode_console_write(
    ctx: &mut SyscallCtx,
    args: [u64; 6],
) -> Result<DecodedSyscall<ConsoleWriteRequest, OptionalOutValue<usize>>, zx_status_t> {
    let len = ctx.arg_usize_or(args, 1, ZX_ERR_OUT_OF_RANGE)?;
    let buffer = ctx.arg_const_ptr::<u8>(args, 0);
    ...
    Ok(DecodedSyscall::new(
        ...
        ctx.decode_optional_out_value::<usize>(args, 2)?,
    ))
}

fn run_console_write(req: ConsoleWriteRequest) -> Result<usize, zx_status_t> {
    Ok(crate::arch::x86_64::serial::write_bytes(&req.bytes))
}
...
typed_syscall!(
    ...
    usize,
    decode_console_write,
    run_console_write,
    writeback_optional_usize
);
const CONSOLE_WRITE_DISPATCH: SyscallDispatch = SyscallDispatch::new(console_write_entry);

#[derive(Debug)]
struct ConsoleReadRequest {
    len: usize,
}

fn decode_console_read(
    ctx: &mut SyscallCtx,
    args: [u64; 6],
) -> Result<
    DecodedSyscall<ConsoleReadRequest, (UserWriteBytes, OptionalOutValue<usize>)>,
    zx_status_t,
> {
    let len = ctx.arg_usize_or(args, 1, ZX_ERR_OUT_OF_RANGE)?;
    let out = ctx.decode_user_write_bytes(ctx.arg_ptr::<u8>(args, 0), len)?;
    Ok(DecodedSyscall::new(
        ConsoleReadRequest { len },
        (out, ctx.decode_optional_out_value::<usize>(args, 2)?),
    ))
}

fn run_console_read(req: ConsoleReadRequest) -> Result<Vec<u8>, zx_status_t> {
    Ok(crate::arch::x86_64::serial::read_bytes(req.len))
}

fn writeback_console_read(
    _ctx: &mut SyscallCtx,
    out: (UserWriteBytes, OptionalOutValue<usize>),
    bytes: Vec<u8>,
) -> Result<(), zx_status_t> {
    write_user_bytes(out.0, &bytes)?;
    write_optional_out_value(out.1, bytes.len())
}
...
typed_syscall!(
    ...
    console_read_entry,
    ConsoleReadRequest,
    (UserWriteBytes, OptionalOutValue<usize>),
    Vec<u8>,
    decode_console_read,
    run_console_read,
    writeback_console_read
);
const CONSOLE_READ_DISPATCH: SyscallDispatch = SyscallDispatch::new(console_read_entry);
...
type HandleDuplicateRevocableRequest = (zx_handle_t, zx_rights_t, zx_handle_t);

fn decode_handle_duplicate_revocable(
    ctx: &mut SyscallCtx,
    args: [u64; 6],
) -> Result<DecodedSyscall<HandleDuplicateRevocableRequest, OutValue<zx_handle_t>>, zx_status_t> {
    Ok(DecodedSyscall::new(
        (
            ctx.arg_handle(args, 0)?,
            ctx.arg_u32(args, 1)? as zx_rights_t,
...
fn run_handle_duplicate_revocable(
    req: HandleDuplicateRevocableRequest,
) -> Result<zx_handle_t, zx_status_t> {
    crate::object::handle::duplicate_handle_revocable(req.0, req.1, req.2)
}

typed_syscall!(
    HANDLE_DUPLICATE_REVOCABLE_TYPED,
    handle_duplicate_revocable_entry,
    HandleDuplicateRevocableRequest,
    OutValue<zx_handle_t>,
    zx_handle_t,
    decode_handle_duplicate_revocable,
    run_handle_duplicate_revocable,
    writeback_handle
);
const HANDLE_DUPLICATE_REVOCABLE_DISPATCH: SyscallDispatch =
    SyscallDispatch::new(handle_duplicate_revocable_entry);

fn decode_revocation_group_create(
    ctx: &mut SyscallCtx,
    args: [u64; 6],
...
fn run_revocation_group_create(options: u32) -> Result<zx_handle_t, zx_status_t> {
    crate::object::revocation::create_revocation_group(options)
}

typed_syscall!(
    REVOCATION_GROUP_CREATE_TYPED,
    revocation_group_create_entry,
    u32,
    OutValue<zx_handle_t>,
    zx_handle_t,
    decode_revocation_group_create,
    run_revocation_group_create,
    writeback_handle
);
const REVOCATION_GROUP_CREATE_DISPATCH: SyscallDispatch =
    SyscallDispatch::new(revocation_group_create_entry);

fn decode_revocation_group_get_info(
    ctx: &mut SyscallCtx,
    args: [u64; 6],
) -> Result<DecodedSyscall<zx_handle_t, OutValue<ax_revocation_group_info_t>>, zx_status_t> {
    Ok(DecodedSyscall::new(
        ctx.arg_handle(args, 0)?,
        ctx.decode_out_value::<ax_revocation_group_info_t>(args, 1)?,
    ))
}

fn run_revocation_group_get_info(
    handle: zx_handle_t,
) -> Result<ax_revocation_group_info_t, zx_status_t> {
    crate::object::revocation::revocation_group_get_info(handle)
}

fn writeback_revocation_group_info(
    _ctx: &mut SyscallCtx,
    out: OutValue<ax_revocation_group_info_t>,
    info: ax_revocation_group_info_t,
) -> Result<(), zx_status_t> {
    write_out_value(out, info)
}

typed_syscall!(
    REVOCATION_GROUP_GET_INFO_TYPED,
    revocation_group_get_info_entry,
    zx_handle_t,
    OutValue<ax_revocation_group_info_t>,
    ax_revocation_group_info_t,
    decode_revocation_group_get_info,
    run_revocation_group_get_info,
    writeback_revocation_group_info
);
const REVOCATION_GROUP_GET_INFO_DISPATCH: SyscallDispatch =
    SyscallDispatch::new(revocation_group_get_info_entry);

fn decode_revocation_group_revoke(
    ctx: &mut SyscallCtx,
    args: [u64; 6],
) -> Result<DecodedSyscall<zx_handle_t, NoWriteback>, zx_status_t> {
    Ok(DecodedSyscall::new(ctx.arg_handle(args, 0)?, NoWriteback))
...
fn run_revocation_group_revoke(handle: zx_handle_t) -> Result<(), zx_status_t> {
    crate::object::revocation::revocation_group_revoke(handle)
}

typed_syscall!(
    REVOCATION_GROUP_REVOKE_TYPED,
    revocation_group_revoke_entry,
    zx_handle_t,
    NoWriteback,
    (),
    decode_revocation_group_revoke,
    run_revocation_group_revoke,
    writeback_noop
);
const REVOCATION_GROUP_REVOKE_DISPATCH: SyscallDispatch =
    SyscallDispatch::new(revocation_group_revoke_entry);
...
type PortGetInfoRequest = zx_handle_t;

fn decode_port_get_info(
    ctx: &mut SyscallCtx,
    args: [u64; 6],
) -> Result<DecodedSyscall<PortGetInfoRequest, OutValue<ax_port_info_t>>, zx_status_t> {
    Ok(DecodedSyscall::new(
        ctx.arg_handle(args, 0)?,
        ctx.decode_out_value::<ax_port_info_t>(args, 1)?,
    ))
}

fn run_port_get_info(req: PortGetInfoRequest) -> Result<ax_port_info_t, zx_status_t> {
    crate::object::port_get_info(req)
}

fn writeback_port_info(
    _ctx: &mut SyscallCtx,
    out: OutValue<ax_port_info_t>,
    info: ax_port_info_t,
) -> Result<(), zx_status_t> {
    write_out_value(out, info)
}
...
typed_syscall!(
    ...
    port_get_info_entry,
    PortGetInfoRequest,
    OutValue<ax_port_info_t>,
    ax_port_info_t,
    decode_port_get_info,
    run_port_get_info,
    writeback_port_info
);
const PORT_GET_INFO_DISPATCH: SyscallDispatch = SyscallDispatch::new(port_get_info_entry);
...
fn writeback_port_wait(
    ...
    match response {
        ...
        crate::wait::PortWaitOutcome::Resumed => Ok(()),
...
type VmarGetMappingVmoRequest = (zx_handle_t, u64);
...
fn decode_ax_vmar_get_mapping_vmo(
    ctx: &mut SyscallCtx,
    args: [u64; 6],
) -> Result<DecodedSyscall<VmarGetMappingVmoRequest, OutValue<zx_handle_t>>, zx_status_t> {
    Ok(DecodedSyscall::new(
        (ctx.arg_handle(args, 0)?, args[1]),
        ctx.decode_out_value::<zx_handle_t>(args, 2)?,
    ))
...
fn run_ax_vmar_get_mapping_vmo(req: VmarGetMappingVmoRequest) -> Result<zx_handle_t, zx_status_t> {
    crate::object::vm::vmar_get_mapping_vmo(req.0, req.1)
}

typed_syscall!(
    AX_VMAR_GET_MAPPING_VMO_TYPED,
    ax_vmar_get_mapping_vmo_entry,
    VmarGetMappingVmoRequest,
    OutValue<zx_handle_t>,
    zx_handle_t,
    decode_ax_vmar_get_mapping_vmo,
    run_ax_vmar_get_mapping_vmo,
    writeback_handle
);
const AX_VMAR_GET_MAPPING_VMO_DISPATCH: SyscallDispatch =
    SyscallDispatch::new(ax_vmar_get_mapping_vmo_entry);
...
type VmoCreatePrivateCloneRequest = zx_handle_t;

fn decode_ax_vmo_create_private_clone(
    ctx: &mut SyscallCtx,
    args: [u64; 6],
) -> Result<DecodedSyscall<VmoCreatePrivateCloneRequest, OutValue<zx_handle_t>>, zx_status_t> {
    Ok(DecodedSyscall::new(
        ctx.arg_handle(args, 0)?,
        ctx.decode_out_value::<zx_handle_t>(args, 1)?,
    ))
...
fn run_ax_vmo_create_private_clone(
    req: VmoCreatePrivateCloneRequest,
) -> Result<zx_handle_t, zx_status_t> {
    crate::object::vm::vmo_create_private_clone(req)
}

typed_syscall!(
    AX_VMO_CREATE_PRIVATE_CLONE_TYPED,
    ax_vmo_create_private_clone_entry,
    VmoCreatePrivateCloneRequest,
    OutValue<zx_handle_t>,
    zx_handle_t,
    decode_ax_vmo_create_private_clone,
    run_ax_vmo_create_private_clone,
    writeback_handle
);
const AX_VMO_CREATE_PRIVATE_CLONE_DISPATCH: SyscallDispatch =
    SyscallDispatch::new(ax_vmo_create_private_clone_entry);
...
type PciDeviceSetCommandRequest = (zx_handle_t, u16);

fn decode_ax_pci_device_set_command(
    ctx: &mut SyscallCtx,
    args: [u64; 6],
) -> Result<DecodedSyscall<PciDeviceSetCommandRequest, NoWriteback>, zx_status_t> {
    Ok(DecodedSyscall::new(
        (ctx.arg_handle(args, 0)?, ctx.arg_u32(args, 1)? as u16),
        NoWriteback,
    ))
}

fn run_ax_pci_device_set_command(req: PciDeviceSetCommandRequest) -> Result<(), zx_status_t> {
    crate::object::device::pci_device_set_command(req.0, req.1)
}

typed_syscall!(
    AX_PCI_DEVICE_SET_COMMAND_TYPED,
    ax_pci_device_set_command_entry,
    PciDeviceSetCommandRequest,
    NoWriteback,
    (),
    decode_ax_pci_device_set_command,
    run_ax_pci_device_set_command,
    writeback_noop
);
const AX_PCI_DEVICE_SET_COMMAND_DISPATCH: SyscallDispatch =
    SyscallDispatch::new(ax_pci_device_set_command_entry);
...
fn syscall_dispatch(nr: SyscallNumber) -> Option<&'static SyscallDispatch> {
    match nr {
        ...
        AXLE_SYS_AX_HANDLE_DUPLICATE_REVOCABLE => Some(&HANDLE_DUPLICATE_REVOCABLE_DISPATCH),
        ...
        AXLE_SYS_AX_PORT_GET_INFO => Some(&PORT_GET_INFO_DISPATCH),
        ...
        AXLE_SYS_AX_VMAR_GET_MAPPING_VMO => Some(&AX_VMAR_GET_MAPPING_VMO_DISPATCH),
        AXLE_SYS_AX_VMO_CREATE_PRIVATE_CLONE => Some(&AX_VMO_CREATE_PRIVATE_CLONE_DISPATCH),
        ...
        AXLE_SYS_AX_PCI_DEVICE_SET_COMMAND => Some(&AX_PCI_DEVICE_SET_COMMAND_DISPATCH),
        ...
        AXLE_SYS_AX_CONSOLE_WRITE => Some(&CONSOLE_WRITE_DISPATCH),
        AXLE_SYS_AX_CONSOLE_READ => Some(&CONSOLE_READ_DISPATCH),
