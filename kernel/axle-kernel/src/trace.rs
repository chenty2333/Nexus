...
static TRACE_SYS_NATIVE_SYSRET_PHASE1: AtomicU64 = AtomicU64::new(0);
...
pub(crate) fn bootstrap_trace_sys_native_sysret_phase1() -> u64 {
    TRACE_SYS_NATIVE_SYSRET_PHASE1.load(Ordering::Acquire)
}
...
pub(crate) fn init_bootstrap_trace() {
    ...
    TRACE_SYS_NATIVE_SYSRET_PHASE1.store(0, Ordering::Release);
...
pub(crate) fn record_sys_native_sysret(syscall_nr: u64, status: axle_types::zx_status_t) {
    record(
        TraceCategory::Syscall,
        TraceEvent::SysNativeSysret,
        syscall_nr,
