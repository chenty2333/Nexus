...
/// Install a user-callable interrupt gate for vector 0x80 and load the IDT.
pub fn init_int80_gate(handler: usize) {
    // Type attrs:
    // - P=1
    // - DPL=3 (callable from CPL3)
    // - Gate type=0xE (interrupt gate)
    let type_attr: u8 = 0xEE;
    let selector = current_cs();

    // SAFETY: we are in single-core early bring-up; mutating the static IDT table is serialized.
    unsafe {
        IDT[0x80] = IdtEntry::new(handler, selector, type_attr);
    }
    load_idt();
}
