...
starnix-shell:
  nix develop -c bash -lc 'set -euo pipefail; target_dir=target/starnix-shell; cargo build -p axle-kernel --target x86_64-unknown-none --target-dir "$target_dir"; NEXUS_INIT_ROOT_URL=boot://root-starnix-shell RUSTFLAGS="-C code-model=large -C debuginfo=0" cargo build -p nexus-init --target x86_64-unknown-none --target-dir "$target_dir"; init="$target_dir/x86_64-unknown-none/debug/nexus-init"; init_size=$(stat -c%s "$init"); exec qemu-system-x86_64 -machine q35 -m 256M -smp 2 -nographic -serial stdio -monitor none -no-reboot -device isa-debug-exit,iobase=0xf4,iosize=0x04 -device loader,file="$init",addr=0x7000000,force-raw=on -device loader,data=$init_size,data-len=8,addr=0x6fffff8 -device loader,file="$init",addr=0x5000000,force-raw=on -device loader,data=$init_size,data-len=8,addr=0x4fffff8 -kernel "$target_dir/x86_64-unknown-none/debug/axle-kernel"'
