...
perf-smoke-kvm:
  bash -lc 'set -euo pipefail; target_dir=target/perf-smoke-bundle; if [ ! -f "$target_dir/axle-kernel" ] || [ ! -f "$target_dir/nexus-test-runner" ]; then just perf-smoke-bundle >/dev/null; fi; run_dir=target/perf-smoke-kvm; mkdir -p "$run_dir"; runner="$run_dir/nexus-test-runner"; cp "$target_dir/nexus-test-runner" "$runner"; size=$(stat -c%s "$runner"); log="$run_dir/serial.log"; set +e; qemu-system-x86_64 -machine q35,accel=kvm -cpu host -m 256M -smp 2 -nographic -serial file:"$log" -monitor none -no-reboot -device isa-debug-exit,iobase=0xf4,iosize=0x04 -device loader,file="$runner",addr=0x7000000,force-raw=on -device loader,data=$size,data-len=8,addr=0x6fffff8 -kernel "$target_dir/axle-kernel"; qemu_code=$?; set -e; printf "serial_log=%s\nqemu_exit=%s\n" "$log" "$qemu_code"; [ "$qemu_code" -eq 33 ]; python tools/axle-conformance/scripts/extract_perf_smoke.py "$log" > "$run_dir/perf-smoke.json"; python tools/axle-conformance/scripts/perf_smoke_baseline.py "$log" > "$run_dir/baseline.json"; cat "$run_dir/baseline.json"'
