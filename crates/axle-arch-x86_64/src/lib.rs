...
//! the userspace syscall trampolines used by Axle's native path and early
//! conformance runner.
...
/// Invoke Axle's native x86_64 syscall ABI from userspace.
///
/// Arguments map to the current calling convention:
/// - `rax` = syscall number
/// - `rdi/rsi/rdx/r10/r8/r9` = args 0..5
...
/// Returns the raw `zx_status_t` encoded in `rax`.
#[inline(always)]
pub fn native_syscall(nr: u64, args: [u64; 6]) -> i32 {
    let ret_rax: u64;

    // SAFETY: executes Axle's native x86_64 syscall ABI. Register assignments
    // follow the documented contract, and `syscall` is allowed to clobber
    // `rcx`/`r11`.
    unsafe {
        core::arch::asm!(
            "syscall",
...
/// Invoke Axle's native x86_64 syscall ABI with two extra stack arguments.
///
/// Arguments map to the current calling convention:
/// - `rax` = syscall number
/// - `rdi/rsi/rdx/r10/r8/r9` = args 0..5
/// - `*(rsp + 0)` and `*(rsp + 8)` = args 6..7
#[inline(always)]
pub fn native_syscall8(nr: u64, args: [u64; 8]) -> i32 {
    let ret_rax: u64;

    // SAFETY: executes Axle's native syscall ABI. The first six arguments are
    // passed in the documented register set and args 6..7 are staged on the
    // userspace stack where the kernel trap handler reads them.
    unsafe {
        core::arch::asm!(
            "sub rsp, 16",
            "mov [rsp + 0], {arg6}",
