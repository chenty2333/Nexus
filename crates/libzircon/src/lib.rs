...
use axle_types::syscall_numbers::{
    ...
    AXLE_SYS_INTERRUPT_CREATE, AXLE_SYS_INTERRUPT_GET_INFO, AXLE_SYS_INTERRUPT_MASK,
...
pub fn ax_console_write(bytes: &[u8], out_actual: &mut usize) -> zx_status_t {
    native_call(
        AXLE_SYS_AX_CONSOLE_WRITE as u64,
        [
            bytes.as_ptr() as u64,
            bytes.len() as u64,
            out_actual as *mut usize as u64,
            0,
            0,
            0,
        ],
...
pub fn ax_console_read(buffer: &mut [u8], out_actual: &mut usize) -> zx_status_t {
    native_call(
        AXLE_SYS_AX_CONSOLE_READ as u64,
        [
...
pub fn ax_port_get_info(handle: zx_handle_t, out: &mut zx_port_info_t) -> zx_status_t {
    native_call(
...
pub fn ax_vmo_create_private_clone(handle: zx_handle_t, out: &mut zx_handle_t) -> zx_status_t {
    native_call(
        AXLE_SYS_AX_VMO_CREATE_PRIVATE_CLONE as u64,
        [handle, out as *mut zx_handle_t as u64, 0, 0, 0, 0],
    )
}

/// Promote one local-private VMO object to the shared/global backing domain.
pub fn ax_vmo_promote_shared(handle: zx_handle_t) -> zx_status_t {
    native_call(
        AXLE_SYS_AX_VMO_PROMOTE_SHARED as u64,
        [handle, 0, 0, 0, 0, 0],
    )
}

/// Clone all child-visible source mappings from one VMAR into another.
pub fn ax_vmar_clone_mappings(src_vmar: zx_handle_t, dst_vmar: zx_handle_t) -> zx_status_t {
    native_call(
...
pub fn ax_dma_region_get_info(
    ...
    native_call(
        AXLE_SYS_AX_DMA_REGION_GET_INFO as u64,
        [
            handle,
            out_info as *mut zx_dma_region_info_t as u64,
            0,
            0,
            0,
            0,
...
pub fn ax_dma_region_get_segment(
    handle: zx_handle_t,
    segment_index: u32,
    out_info: &mut zx_dma_segment_info_t,
) -> zx_status_t {
    native_call(
        AXLE_SYS_AX_DMA_REGION_GET_SEGMENT as u64,
        [
            handle,
            segment_index as u64,
            out_info as *mut zx_dma_segment_info_t as u64,
            0,
            0,
            0,
        ],
...
pub fn ax_pci_device_get_config(
    handle: zx_handle_t,
    out_info: &mut zx_pci_config_info_t,
) -> zx_status_t {
    native_call(
        AXLE_SYS_AX_PCI_DEVICE_GET_CONFIG as u64,
        [
            handle,
            out_info as *mut zx_pci_config_info_t as u64,
            0,
            0,
            0,
            0,
...
pub fn ax_pci_device_get_interrupt_mode(
    handle: zx_handle_t,
    mode: u32,
    out_info: &mut zx_pci_interrupt_mode_info_t,
) -> zx_status_t {
    native_call(
        AXLE_SYS_AX_PCI_DEVICE_GET_INTERRUPT_MODE as u64,
        [
            handle,
            mode as u64,
            out_info as *mut zx_pci_interrupt_mode_info_t as u64,
            0,
            0,
            0,
        ],
...
pub fn ax_pci_device_get_resource_count(handle: zx_handle_t, out_count: &mut u64) -> zx_status_t {
    native_call(
        AXLE_SYS_AX_PCI_DEVICE_GET_RESOURCE_COUNT as u64,
        [handle, out_count as *mut u64 as u64, 0, 0, 0, 0],
    )
}
...
pub fn ax_pci_device_get_resource(
    handle: zx_handle_t,
    resource_index: u32,
    out_info: &mut zx_pci_resource_info_t,
) -> zx_status_t {
    native_call(
        AXLE_SYS_AX_PCI_DEVICE_GET_RESOURCE as u64,
        [
            handle,
            resource_index as u64,
            out_info as *mut zx_pci_resource_info_t as u64,
            0,
            0,
            0,
        ],
...
pub fn ax_pci_device_set_interrupt_mode(handle: zx_handle_t, mode: u32) -> zx_status_t {
    native_call(
        AXLE_SYS_AX_PCI_DEVICE_SET_INTERRUPT_MODE as u64,
        [handle, mode as u64, 0, 0, 0, 0],
    )
}

/// Update the live PCI command register for a PCI/device handle.
pub fn ax_pci_device_set_command(handle: zx_handle_t, command: u16) -> zx_status_t {
    native_call(
        AXLE_SYS_AX_PCI_DEVICE_SET_COMMAND as u64,
        [handle, u64::from(command), 0, 0, 0, 0],
    )
}
