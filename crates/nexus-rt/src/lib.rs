//! Minimal single-thread userspace reactor built on Zircon-style ports.
//!
//! This is intentionally small. The current goal is to provide just enough
//! runtime glue for userspace services to block on `port_wait` and dispatch
//! `wait_async` signal packets without busy-waiting.
