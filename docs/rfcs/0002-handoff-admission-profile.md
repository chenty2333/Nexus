...
8. the matrix and v2 receipt both state
   `production_registry_modified=true` for the complete gate's production
   `EffectRegistry` refinement lane, while oracle independence is recorded in
   its own typed receipt field,
