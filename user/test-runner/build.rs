fn main() {
    ...
    println!(
        "cargo:rustc-check-cfg=cfg(axle_test_runner_rust_entry, values(\"reactor_smoke\", \"component_smoke\", \"perf_smoke\", \"device_smoke\", \"net_smoke\", \"datagram_smoke\", \"port_smoke\", \"revocation_smoke\", \"job_smoke\", \"smp_smoke\", \"vmo_info_smoke\", \"vmo_shared_smoke\", \"vmo_promotion_smoke\", \"vmo_private_clone_smoke\", \"vmo_private_object_clone_smoke\"))"
    );
    ...
    if let Ok(entry) = std::env::var("AXLE_TEST_RUNNER_RUST_ENTRY") {
        match entry.as_str() {
            ...
            "vmo_private_object_clone_smoke" => {
                println!("cargo:rustc-cfg=axle_test_runner_rust_entry=\"{entry}\"");
            }
