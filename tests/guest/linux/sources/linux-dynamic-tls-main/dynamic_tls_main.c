...
__thread int tls_value = 0x12345678;
...
__attribute__((noreturn, visibility("hidden"))) void dynamic_tls_start(u64 *stack) {
    static const char ok[] = "dynamic tls ok\n";
    u64 aux = 0;

    if (tls_value != 0x12345678) {
        sys_exit(2);
    }
    tls_value = 0x87654321;
    if (tls_value != 0x87654321) {
        sys_exit(3);
    }
    ...
    if (!find_aux(stack, 31, &aux)
        || !string_equals((const char *)aux, "/bin/linux-dynamic-tls-main")) {
        sys_exit(7);
    }
