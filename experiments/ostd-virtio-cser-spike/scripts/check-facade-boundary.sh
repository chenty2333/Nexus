...
for source in pci dma portal production; do
...
if grep -Eq '^[[:space:]]*(mod (pci|dma|portal|production)|unsafe[[:space:]])' \
...
[[ $(grep -Fc '#[allow(unsafe_code)]' "$facade_root/src/lib.rs") -eq 4 ]] \
    || fail 'unsafe allowance is not confined to exactly four private modules'
for module in dma pci portal production; do
...
if grep -REn '^[[:space:]]*pub[[:space:]]+unsafe[[:space:]]+fn|^[[:space:]]*pub[[:space:]]+mod[[:space:]]+(pci|dma|portal|production)' \
...
echo 'OSTD VirtIO facade boundary: PASS client_unsafe=false raw_api=false opaque_root=true substrate_modules=4 production_authority=external experiment_equivalence=runtime_oracle kernel_default=disabled'
